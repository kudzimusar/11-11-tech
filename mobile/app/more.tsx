...
export default function MoreScreen() {
  ...
  const openExternal = (url: string) => {
    Linking.openURL(url).catch(() => Alert.alert('Could not open link', 'This link is not available on the device right now.'))
  }
  ...
  return (
    <Screen contentStyle={styles.screen}>
      <View style={styles.top}>
        ...
        <PressableScale accessibilityLabel="Close menu" accessibilityHint="Returns to the previous screen" onPress={() => router.back()} style={styles.close}><Text style={styles.closeText}>×</Text></PressableScale>
      </View>
...
function MenuRow({ index, label, onPress, accent = false }: { index: string; label: string; onPress: () => void; accent?: boolean }) {
  return (
    <PressableScale accessibilityLabel={label} onPress={onPress} style={[styles.menuRow, accent && styles.menuRowAccent]}>
...
const styles = StyleSheet.create({
  screen: { minHeight: '100%', justifyContent: 'space-between', gap: spacing.xxl },
  ...
  close: { width: 48, height: 48, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.ruleDark, alignItems: 'center', justifyContent: 'center' },
  ...
  secondaryRow: { minHeight: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
