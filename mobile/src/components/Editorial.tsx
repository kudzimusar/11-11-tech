...
export function CinematicHero({ image, eyebrow, title, body, footer }: { image: string; eyebrow: string; title: string; body: string; footer?: ReactNode }) {
  ...
  return <View style={[styles.hero, { minHeight: width < 360 ? 430 : 500 }]}>
    ...
    <View style={styles.heroCopy}><Text style={styles.heroEyebrow}>{eyebrow.toUpperCase()}</Text><Text maxFontSizeMultiplier={1.25} style={[styles.heroTitle,{fontSize:width<360?44:56,lineHeight:width<360?43:53}]}>{title.toUpperCase()}</Text><Text style={styles.heroBody}>{body}</Text>{footer}</View>
  </View>
}
...
export function MediaCard({ image, label, title, detail, onPress }: { image: string; label: string; title: string; detail?: string; onPress?: () => void }) {
  return <PressableScale accessibilityLabel={`${title}. ${detail ?? ''}`} onPress={onPress ?? (()=>undefined)} style={styles.mediaCard} haptic={onPress ? 'light' : undefined}>
    <Image source={{uri:image}} style={styles.mediaImage} resizeMode="cover"/><View style={styles.mediaShade}/><View style={styles.mediaCopy}><Text style={styles.mediaLabel}>{label.toUpperCase()}</Text><Text style={styles.mediaTitle}>{title}</Text>{detail?<Text style={styles.mediaDetail}>{detail}</Text>:null}</View>
  </PressableScale>
}
