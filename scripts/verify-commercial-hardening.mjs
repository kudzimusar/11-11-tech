...
assert(edge.includes("Deno.env.get('IP_HASH_SALT') || Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')") && edge.includes("name: 'HMAC'") && edge.includes("IP hash secret is not configured"), 'IP hashing must use a private server-side HMAC secret and fail closed if no server secret exists')
