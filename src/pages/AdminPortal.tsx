...
import { callCommercialAdmin, loadAdminData, type AdminData } from '../lib/adminOps'
...
export function AdminPortal() {
  ...
  const createProject = async (event: FormEvent) => {
    ...
    try {
      await callCommercialAdmin('create-project', {
        organizationName: projectForm.organizationName,
        billingEmail: projectForm.billingEmail,
        clientName: projectForm.clientName,
        title: projectForm.title,
        serviceCategory: projectForm.serviceCategory,
  ...
  return <section className="commercial-shell admin-shell">
    <aside className="commercial-rail admin-rail">
      <a className="commercial-logo" href={hrefFor('/')}><img src={hrefFor('/assets/logo-light.svg')} alt="11-11 Tech" /></a>
      <div className="commercial-context"><small>COMPANY ADMIN</small><strong>Commercial OS</strong><span>Clients · contracts · money</span></div>
      <nav aria-label="Company admin">
        {([['overview','Overview'],['leads','Leads'],['clients','Clients'],['projects','Projects'],['billing','Billing'],['documents','Documents']] as [Tab,string][]).map(([value,label]) => <button key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>{label}{value === 'leads' && <span>{data.leads.filter((lead) => lead.status === 'new').length}</span>}</button>)}
      </nav>
      <div className="commercial-rail-bottom"><a href={hrefFor('/')}>← Public website</a><button onClick={() => { signOutCommercial(); setData(empty) }}>Sign out</button></div>
    </aside>

    <main className="commercial-main">
      <header className="commercial-topbar"><div><small>11-11 TECH · ADMIN</small><h1>{tab === 'overview' ? 'Commercial command centre' : tab[0].toUpperCase() + tab.slice(1)}</h1></div><button className="btn primary" onClick={() => setProjectDialog(true)}>New client project +</button></header>
      {error && <div className="commercial-alert error" role="alert">{error}</div>}
      {notice && <div className="commercial-alert" role="status">{notice}</div>}
      {loading && <div className="commercial-loading">Loading company commercial data…</div>}
      ...
      {!loading && tab === 'projects' && <DataTable title="Project register" intro="Project value, lifecycle and tenant ownership stay anchored here." columns={['Reference','Project','Client','Value','Status']} rows={data.projects.map((item) => [item.reference, item.title, orgById.get(item.organization_id)?.trading_name || orgById.get(item.organization_id)?.legal_name || 'Client', money(item.contract_value_minor,item.currency), item.status])}/>} 
