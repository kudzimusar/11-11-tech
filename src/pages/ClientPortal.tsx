...
export function ClientPortal() {
  ...
  const [tab, setTab] = useState<'overview' | 'documents' | 'billing' | 'agreement'>(params.get('reference') ? 'agreement' : 'overview')
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const requestedProject = requestedReference ? result.projects.find((project) => project.reference.toLowerCase() === requestedReference) : undefined
  ...
  return <section className="commercial-shell"><aside className="commercial-rail"><a className="commercial-logo" href={hrefFor('/')}><img src={hrefFor('/assets/logo-light.svg')} alt="11-11 Tech" /></a><div className="commercial-context"><small>CLIENT WORKSPACE</small><strong>{organization?.trading_name || organization?.legal_name || 'Your account'}</strong><span>Secure commercial record</span></div><nav aria-label="Client workspace"><button className={tab==='overview'?'active':''} onClick={()=>setTab('overview')}>Overview</button><button className={tab==='documents'?'active':''} onClick={()=>setTab('documents')}>Documents <span>{documents.length}</span></button><button className={tab==='billing'?'active':''} onClick={()=>setTab('billing')}>Billing</button><button className={tab==='agreement'?'active':''} onClick={()=>setTab('agreement')}>Agreement & pay</button></nav><div className="commercial-rail-bottom"><a href={hrefFor('/')}>← Public website</a><button onClick={()=>{signOutCommercial();setData(emptyData);setSessionReady(false)}}>Sign out</button></div></aside>
    <main className="commercial-main"><header className="commercial-topbar"><div><small>11-11 TECH · CLIENT</small><h1>{project?.title || 'Client workspace'}</h1></div>{data.projects.length>1&&<select aria-label="Choose project" value={project?.id??''} onChange={(event)=>setActiveProjectId(event.target.value)}>{data.projects.map((item)=><option value={item.id} key={item.id}>{item.reference} · {item.title}</option>)}</select>}</header>{error&&<div className="commercial-alert error" role="alert">{error}</div>}{message&&<div className="commercial-alert" role="status">{message}</div>}{loading&&<div className="commercial-loading">Loading your secure workspace…</div>}{!loading&&!project&&<EmptyWorkspace/>}
      ...
      {project&&tab==='billing'&&<Billing project={project} invoices={invoices} payments={payments} selectedPlan={selectedPlan} installments={data.installments} paid={paidMinor} outstanding={outstandingMinor} onPay={()=>setTab('agreement')}/>} 
      {project&&tab==='agreement'&&<AgreementAndPayment project={project} plans={projectPlans} planChoice={selectedPlan?.id??planChoice} setPlanChoice={setPlanChoice} documents={requiredDocuments} previouslyAccepted={previouslyAccepted} opened={openedDocuments} checked={acceptedChecks} toggleCheck={(id:string)=>setAcceptedChecks((current)=>{const next=new Set(current);next.has(id)?next.delete(id):next.add(id);return next})} onOpen={openDocument} selectedPlan={selectedPlan} autoPayAccepted={alreadyAuthorized||autoPayAccepted} setAutoPayAccepted={setAutoPayAccepted} ready={checkoutReady} processing={processing} paymentsConfigured={paymentsConfigured} onContinue={continueToPayment}/>} 
    </main></section>
}
...
function Billing({project,invoices,payments,selectedPlan,installments,paid,outstanding,onPay}:any){return <div className="commercial-stack"><section className="commercial-section"><div className="commercial-section-heading"><div><small>BILLING</small><h2>Paid, owing and due next.</h2></div><button className="btn primary" disabled={outstanding<=0} onClick={onPay}>{outstanding>0?'Make a payment ↗':'Paid'}</button></div><div className="billing-balance"><div><span>Paid</span><strong>{money(paid,project.currency)}</strong></div><div><span>Outstanding</span><strong>{money(outstanding,project.currency)}</strong></div><div><span>Project value</span><strong>{money(project.contract_value_minor,project.currency)}</strong></div></div></section>{selectedPlan&&<section className="commercial-section"><div className="commercial-section-heading"><div><small>PAYMENT PLAN</small><h2>{selectedPlan.name}</h2></div><span className="status-pill active">{selectedPlan.status}</span></div><div className="installment-list">{installments.filter((i:any)=>i.payment_plan_id===selectedPlan.id).map((i:any)=><div key={i.id}><span>{String(i.sequence_no).padStart(2,'0')}</span><strong>{money(i.amount_minor,selectedPlan.currency)}</strong><small>{shortDate(i.due_at)}</small><em>{i.status.replaceAll('_',' ')}</em></div>)}</div></section>}<section className="commercial-two-col"><article className="commercial-section"><small>INVOICES</small><div className="compact-ledger">{invoices.length?invoices.map((i:any)=><div key={i.id}><span><strong>{i.reference}</strong><small>{shortDate(i.due_at)}</small></span><span><strong>{money(i.amount_due_minor,i.currency)}</strong><small>{i.status.replaceAll('_',' ')}</small></span></div>):<p>No invoice has been issued yet.</p>}</div></article><article className="commercial-section"><small>PAYMENTS & RECEIPTS</small><div className="compact-ledger">{payments.length?payments.map((i:any)=><div key={i.id}><span><strong>{i.receipt_reference||'Payment'}</strong><small>{shortDate(i.received_at||i.created_at)}</small></span><span><strong>{money(i.amount_minor,i.currency)}</strong><small>{i.method.replaceAll('_',' ')}</small></span></div>):<p>No confirmed payment is recorded yet.</p>}</div></article></section></div>}
function AgreementAndPayment({project,plans,planChoice,setPlanChoice,documents,previouslyAccepted,opened,checked,toggleCheck,onOpen,selectedPlan,autoPayAccepted,setAutoPayAccepted,ready,processing,paymentsConfigured,onContinue}:any){return <div className="commercial-stack"><section className="commercial-section"><div className="commercial-section-heading"><div><small>STEP 1 · PAYMENT OPTION</small><h2>Choose how you want to pay.</h2></div><p>Only payment structures approved for this project are shown.</p></div><div className="payment-plan-grid">{plans.map((plan:PortalPlan)=><label key={plan.id} className={planChoice===plan.id?'payment-plan-card selected':'payment-plan-card'}><input type="radio" name="payment-plan" checked={planChoice===plan.id} onChange={()=>setPlanChoice(plan.id)} disabled={['accepted','active'].includes(selectedPlan?.status)&&selectedPlan?.id!==plan.id}/><span><small>{plan.plan_type.replaceAll('_',' ')}</small><strong>{plan.name}</strong><em>{money(plan.total_minor,plan.currency)}</em>{plan.allow_extra_payments&&<p>Additional balance payments are allowed{plan.minimum_extra_payment_minor?` from ${money(plan.minimum_extra_payment_minor,plan.currency)}`:''}.</p>}</span></label>)}</div>{!plans.length&&<div className="commercial-empty-inline">11-11 Tech has not published a payment option for this project yet.</div>}</section><section className="commercial-section"><div className="commercial-section-heading"><div><small>STEP 2 · REVIEW DOCUMENTS</small><h2>Know exactly what you are accepting.</h2></div><p>Open each mandatory document before acknowledging it. You can download and keep the issued PDF.</p></div><div className="agreement-docs">{documents.length?documents.map((doc:PortalDocument)=>{const done=previouslyAccepted.has(doc.id);const wasOpened=opened.has(doc.id)||done;return <article key={doc.id} className={done?'accepted':''}><div className="agreement-doc-title"><span>{done?'✓':'PDF'}</span><div><small>{doc.document_type.replaceAll('_',' ')} · Version {doc.version}</small><h3>{doc.title}</h3><p>{documentPurpose[doc.document_type]||'Applicable 11-11 Tech project terms.'}</p></div></div><button onClick={()=>void onOpen(doc)}>Open document ↗</button><label className="agreement-checkbox"><input type="checkbox" checked={done||checked.has(doc.id)} disabled={done||!wasOpened} onChange={()=>toggleCheck(doc.id)}/><span>{done?'Accepted and recorded.':wasOpened?'I have reviewed this document and agree to the applicable terms.':'Open this document before accepting.'}</span></label></article>}):<div className="commercial-alert error">The required agreement package has not been issued yet. Payment remains locked.</div>}</div></section>{selectedPlan?.requires_autopay_authorization&&<section className="commercial-section autopay-section"><div><small>STEP 3 · PAYMENT AUTHORISATION</small><h2>Scheduled payment authorization.</h2><p>You authorize 11-11 Tech to charge the payment method you provide according to the displayed payment schedule. This does not authorize arbitrary charges outside the accepted plan.</p></div><label className="agreement-checkbox important"><input type="checkbox" checked={autoPayAccepted} onChange={(e)=>setAutoPayAccepted(e.target.checked)}/><span>I authorize the scheduled automatic charges in my selected payment plan.</span></label></section>}<section className="commercial-checkout-card"><img src={hrefFor('/assets/logo-light.svg')} alt="11-11 Tech"/><div><small>FINAL REVIEW</small><h2>{selectedPlan?money(selectedPlan.total_minor,selectedPlan.currency):money(project.contract_value_minor,project.currency)} project agreement</h2><p>Your acceptance is recorded against the exact issued versions. Card details are entered only on Stripe’s secure payment surface.</p><div className="checkout-flags"><span className={documents.length&&documents.every((doc:PortalDocument)=>previouslyAccepted.has(doc.id)||checked.has(doc.id))?'ok':''}>Required documents</span><span className={selectedPlan?'ok':''}>Payment option</span><span className={!selectedPlan?.requires_autopay_authorization||autoPayAccepted?'ok':''}>Payment authorization</span></div></div><button className="btn primary" disabled={!ready||processing||!paymentsConfigured} onClick={()=>void onContinue()}>{processing?'Preparing secure payment…':paymentsConfigured?'Continue to secure payment ↗':'Stripe connection pending'}</button></section></div>}
