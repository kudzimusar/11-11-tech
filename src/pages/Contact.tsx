...
export function Contact() {
  ...
  const summary = useMemo(() => [
    ...
    `Procurement / legal: ${['NDA', inquiry.nda], ['MSA', inquiry.msa], ['DPA', inquiry.dpa], ['Security review', inquiry.security], ['SLA', inquiry.sla], ['Vendor onboarding', inquiry.procurement]].filter((item) => item[1]).map((item) => item[0]).join(', ') || 'None specified'}`,
    '', 'What should change?', inquiry.goals,
  ].join('\n'), [inquiry, reference])

  const mailto = `mailto:${emailAddress}?subject=${encodeURIComponent(`${reference} — ${inquiry.organization || inquiry.name || 'New opportunity'}`)}&body=${encodeURIComponent(summary)}`
