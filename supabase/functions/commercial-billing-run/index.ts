...
Deno.serve(async(req)=>{
  ...
  for(const row of due||[]){
    try{
      ...
      const body=new URLSearchParams({amount:String(remaining),currency:String(plan.currency).toLowerCase(),customer:org.stripe_customer_id,payment_method:pm,off_session:'true',confirm:'true',description:`${project.reference} · installment ${row.sequence_no}`})
