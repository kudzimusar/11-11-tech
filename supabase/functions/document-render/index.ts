...
async function renderDocument(document:any,project:any,organization:any){
  ...
  let logo:any=null;try{const logoBytes=new Uint8Array(await (await fetch(`${publicAppUrl}/assets/icon-512.png`)).arrayBuffer());logo=await pdf.embedPng(logoBytes)}catch{/* wordmark remains */}
...
async function issue(document:any,userId:string){
  ...
  await supabase.from('audit_events').insert({actor_profile_id:userId,organization_id:document.organization_id,project_id:document.project_id,event_type:'document.issued',entity_type:'project_document',entity_id:document.id,context:{version:document.version,sha256:hash,storage_path:path}})
...
async function buildPack(projectId:string,userId:string){
  ...
  const {data:docs,error:dErr}=await supabase.from('project_documents').select('*').eq('project_id',projectId).eq('status','issued').neq('document_type','agreement_pack').order('created_at');if(dErr)throw dErr
  if(!docs?.length)throw new Error('Issue at least one project document before creating the agreement pack.')
...
Deno.serve(async(req)=>{
  ...
  const user=await adminUser(req);if(!user)return json({error:'Company admin access required'},403,origin)
  let input:any;try{input=await req.json()}catch{return json({error:'Invalid request'},400,origin)}
  try{
    if(input?.mode==='pack'){const projectId=uuid(input.projectId);if(!projectId)return json({error:'Project required'},400,origin);return json(await buildPack(projectId,user.id),201,origin)}
    ...
    return json(await issue(document,user.id),201,origin)
